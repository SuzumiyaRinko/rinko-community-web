<template>
  <div class="message">
    <!-- TopNav -->
    <div class="topNav">
      <span class="system" :style="systemStyle" @click="gotoSystem()"
        >MESSAGE</span
      >
      <span class="chat" :style="chatStyle" @click="gotoChat()">CHAT</span>
    </div>
    ...
    <div class="bottomNav">
      ...
      <div class="item currItem" @click="router.push('/message')">
...
export default {
  setup() {
    ...
    const onMessageLoad = async () => {
      ...
      if (
        (messageSelectVO.messages.length > 0 && messagesPage.data.length > 0 && messagesPage.data[0].id != messageSelectVO.messages[0].id)
    ...
    const gotoTarget = async (targetId, systemMsgType, messageId) => {
      ...
      if (
        ...
        // backToSomeone
        window.sessionStorage.setItem("backToSomeone", "message");
        // currPost
