...
export default {
  setup() {
    onMounted(async () => {
      // 本次应该到达的页数
      var homePage = window.sessionStorage.getItem("homePage");
      if(homePage == null) {
        homePage = 1;
      }
      while (postSearchDTO.pageNum <= homePage) {
        onPostLoad();
        await sleep(80);
      }
      // 移动scrollingPost的滚动条
      var homeScroll = window.sessionStorage.getItem("homeScroll");
      document.getElementById("scrollingPost").scrollTop = homeScroll;
    });

    onBeforeRouteLeave((to, from, next) => {
        var scrollTop = document.getElementById("scrollingPost").scrollTop;
        window.sessionStorage.setItem("homeScroll", scrollTop);
        window.sessionStorage.setItem("homePage", postSearchDTO.pageNum - 1);
      next();
    });
