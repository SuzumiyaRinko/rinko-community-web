...
import { search } from "@/api/post.js";
...
export default {
  setup() {
    onMounted(async () => {
      ...
      var baseResponse = (await search(postSearchDTO)).data;
      if (checkAuthority(baseResponse) == false) {
        router.push("/");
      }
      myPosts.value = baseResponse.data.page.data;
    ...
    const onBeforeLogoutClose = async (action) => {
      if (action === "confirm") {
        var baseResponse = (await logout()).data;
